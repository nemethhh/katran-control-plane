...
def _vip(addr, port, proto):
    ...
    return {"address": addr, "port": port, "protocol": proto}
...
class TestVipLifecycle:
    def test_vip_crud(self, api_client):
        ...
        try:
            ...
            # Get
            resp = api_client.get("/api/v1/vips", params=_vip("10.100.0.1", 80, "tcp"))
            assert resp.status_code == 200
            assert resp.json()["address"] == "10.100.0.1"
        finally:
            # Delete
            resp = api_client.post("/api/v1/vips/remove", json=_vip("10.100.0.1", 80, "tcp"))
            assert resp.status_code == 200
    ...
    def test_duplicate_vip_409(self, api_client):
        ...
        try:
            ...
            api_client.post("/api/v1/vips/remove", json=_vip("10.100.0.10", 443, "tcp"))

    def test_get_missing_vip_404(self, api_client):
        resp = api_client.get("/api/v1/vips", params=_vip("10.255.255.255", 9999, "tcp"))
...
class TestBackendLifecycle:
    def test_backend_crud(self, api_client):
        ...
        try:
            ...
            resp = api_client.get("/api/v1/vips", params=vip)
            assert resp.status_code == 200
...
class TestVipErrors:
    def test_delete_nonexistent_vip_404(self, api_client):
        """Remove a VIP that doesn't exist should return 404."""
        resp = api_client.post("/api/v1/vips/remove", json=_vip("10.100.1.1", 9999, "tcp"))
        assert resp.status_code == 404
    ...
    def test_get_vip_invalid_protocol_400(self, api_client):
        """GET VIP with bad protocol in query should return 400."""
        resp = api_client.get("/api/v1/vips", params=_vip("10.100.1.3", 80, "sctp"))
        assert resp.status_code == 400
...
class TestVipCreation:
    def test_vip_with_flags(self, api_client):
        ...
        try:
            ...
            resp = api_client.get("/api/v1/vips", params=_vip("10.100.2.1", 80, "tcp"))
            assert resp.status_code == 200
            assert resp.json()["flags"] == 1
        finally:
            api_client.post("/api/v1/vips/remove", json=_vip("10.100.2.1", 80, "tcp"))
    ...
    def test_udp_vip_creation(self, api_client):
        ...
        try:
            ...
            resp = api_client.get("/api/v1/vips", params=_vip("10.100.2.2", 53, "udp"))
            assert resp.status_code == 200
            assert resp.json()["protocol"] == "udp"
        finally:
            api_client.post("/api/v1/vips/remove", json=_vip("10.100.2.2", 53, "udp"))
...
class TestVipLifecycleExtended:
    def test_vip_recreate_after_delete(self, api_client):
        ...
        try:
            # Delete
            resp = api_client.post("/api/v1/vips/remove", json=_vip("10.100.3.1", 80, "tcp"))
            assert resp.status_code == 200
    ...
    def test_multiple_vips_coexistence(self, api_client):
        ...
        try:
            ...
            # Delete one
            resp = api_client.post("/api/v1/vips/remove", json=_vip("10.100.3.11", 80, "tcp"))
            assert resp.status_code == 200
            ...
            api_client.post("/api/v1/vips/remove", json=_vip("10.100.3.10", 80, "tcp"))
            api_client.post("/api/v1/vips/remove", json=_vip("10.100.3.11", 80, "tcp"))
            api_client.post("/api/v1/vips/remove", json=_vip("10.100.3.12", 80, "tcp"))
...
class TestBackendDetails:
    ...
    def test_shared_backend_across_vips(self, api_client):
        ...
        vip_a = _vip("10.100.5.3", 80, "tcp")
        vip_b = _vip("10.100.5.4", 80, "tcp")

        resp = api_client.post("/api/v1/vips", json=vip_a)
        assert resp.status_code == 201
        resp = api_client.post("/api/v1/vips", json=vip_b)
        assert resp.status_code == 201
        ...
        try:
            ...
            # Delete VIP-A
            resp = api_client.post("/api/v1/vips/remove", json=vip_a)
            assert resp.status_code == 200

            # Backend still exists on VIP-B
            resp = api_client.get("/api/v1/vips", params=vip_b)
            assert resp.status_code == 200
            backends = resp.json()["backends"]
            assert len(backends) == 1
            assert backends[0]["address"] == "10.0.3.20"
        finally:
            api_client.post("/api/v1/vips/remove", json=vip_a)
            api_client.post("/api/v1/vips/remove", json=vip_b)
...
class TestVipLifecycleV6:
    def test_vip_crud_v6(self, api_client):
        ...
        try:
            ...
            # Get
            resp = api_client.get("/api/v1/vips", params=_vip("fc00::1", 80, "tcp"))
            assert resp.status_code == 200
            assert resp.json()["address"] == "fc00::1"
        finally:
            resp = api_client.post("/api/v1/vips/remove", json=_vip("fc00::1", 80, "tcp"))
            assert resp.status_code == 200
    ...
    def test_duplicate_v6_vip_409(self, api_client):
        ...
        try:
            ...
            api_client.post("/api/v1/vips/remove", json=_vip("fc00::10", 443, "tcp"))

    def test_get_missing_v6_vip_404(self, api_client):
        resp = api_client.get("/api/v1/vips", params=_vip("fc00::ffff", 9999, "tcp"))
        assert resp.status_code == 404
...
class TestVipCreationV6:
    def test_udp_v6_vip(self, api_client):
        ...
        try:
            ...
            resp = api_client.get("/api/v1/vips", params=_vip("fc00::30", 53, "udp"))
            assert resp.status_code == 200
            assert resp.json()["protocol"] == "udp"
        finally:
            api_client.post("/api/v1/vips/remove", json=_vip("fc00::30", 53, "udp"))
    ...
    def test_v6_vip_with_flags(self, api_client):
        ...
        try:
            ...
            resp = api_client.get("/api/v1/vips", params=_vip("fc00::31", 80, "tcp"))
            assert resp.status_code == 200
            assert resp.json()["flags"] == 1
        finally:
            api_client.post("/api/v1/vips/remove", json=_vip("fc00::31", 80, "tcp"))
